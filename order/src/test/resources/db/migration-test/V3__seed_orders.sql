-- H2 test version: identical to production but without the Postgres SELECT setval(...) calls,
-- which H2 does not support. No need to fix the sequence since tests only read seed data.
